...
const buttonTwo = {
  content: '<div class="air-datepicker-nav--action"><svg><path d="M 18,10 l -5,6 l 5,6"></path></svg></div>',
...
const buttonThree = {
  content: '<div class="air-datepicker-nav--action"><svg><path d="M 2,10 l 5,6 l -5,6"></path></svg></div>',
  className: 'my--buttons',
  onClick: (dp) => {
    let newDate = new Date(dp.viewDate)
    newDate.setFullYear(dp.viewDate.getFullYear() + 1);
...
const buttonFourth = {
  content: '<div class="air-datepicker-nav--action"><svg><path d="M 14,7 l -5,9 l 5,9"></path></svg></div>',
  className: 'my--buttons',
  onClick: (dp) => {
    let newDate = new Date(dp.viewDate)
...
const datePicker = new AirDatepicker("#calendar", {
  ...
  onShow(animationDone) {
    let header = document.getElementsByClassName("air-datepicker--navigation")[0];
...
const svgs = [...document.getElementById("calendar-section").getElementsByTagName("svg")];
svgs.forEach((it) => it.addEventListener("click", (event) => {
...
// function updateCalendarOptions({ classes }) {
//     datePicker.update({
//         classes: classes
//     })
// }

// export { updateCalendarOptions };
